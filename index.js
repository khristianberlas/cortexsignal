...
const AI_WEBHOOK_URL =
  "https://berlaskhristian.app.n8n.cloud/webhook-test/ask-market";
...
bot.callbackQuery(/select_market_(forex|crypto)_signal/, async (ctx) => {
  ...
  symbols.forEach((symbol, index) => {
    ...
    if ((index + 1) % buttonsPerRow === 0 && index + 1 < symbols.length) {
      keyboard.row(); // Mulai baris baru
    }
  });
...
bot.callbackQuery(/select_symbol_(forex|crypto)_(.+)_signal/, async (ctx) => {
  ...
  TIMEFRAMES.forEach((timeframe, index) => {
    ...
    if ((index + 1) % buttonsPerRow === 0 && index + 1 < TIMEFRAMES.length) {
      keyboard.row(); // Mulai baris baru
    }
  });
